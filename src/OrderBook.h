...
{
public:
	TDF_MARKET_DATA* GetData() const
	{
		return (TDF_MARKET_DATA*)data();
	}

	size_t GetSize() const
	{
		return size() / sizeof(TDF_MARKET_DATA);
	}
};
...
{
public:
	TDF_ORDER_X* GetData() const
	{
		return (TDF_ORDER_X*)data();
	}

	size_t GetSize() const
	{
		return size() / sizeof(TDF_ORDER_X);
	}
};
...
{
public:
	TDF_TRANSACTION_X* GetData() const
	{
		return (TDF_TRANSACTION_X*)data();
	}

	size_t GetSize() const
	{
		return size() / sizeof(TDF_TRANSACTION_X);
	}
};
...
public:
	...
	void SetFrontFence(int64_t price)
	{
		if (compare(price, iFrontFence))
		{
			while (!cache.empty() && !compare(std::prev(cache.end())->first, price))
			{
				...
				{
					if (r.second && (delim == levels.end() || compare(itl->first, delim->first)))
						delim = itl;
	...
	void SetBackFence(int64_t price)
	{
		...
		{
			// Shrink
			while (levels.begin() != delim && compare(price, std::prev(delim)->first))
			{
				--delim;
				--nDepth;
				nTotalVolume -= delim->second;
	...
	void Add(int64_t id, int64_t price, int64_t volume)
	{
		...
		else
		{
			...
			if (ro.second)
			{
				...
				{
					// Active but hidden
					if (r.second && (delim == levels.end() || compare(price, delim->first)))
						delim = itl;
				}
				else
				{
	...
	void Trade(int64_t id, int64_t volume)
	{
		...
		if (!it->second.active)
			SetFrontFence(it->second.itc->first);
	...
	uint32_t HeadDepth() const
	{
		return (uint32_t)cache.size();
	}

	auto HeadBegin() const
	{
		return cache.begin();
	}
	...
	uint32_t TailDepth() const
	{
		return (uint32_t)levels.size() - nDepth;
	}

	auto TailBegin() const
	{
		return delim;
	}
...
// 比较新旧盘口
template <typename Compare>
size_t OBCompare(const int64_t* begNew, size_t newSize,
	const int64_t* begOld, size_t oldSize, int64_t* result, bool showUnchanged)
{
...
private:
	...
	void SZE_Order(const TDF_ORDER_X& order)
	{
		...
		assert(order.chOrderKind == '0' || order.chOrderKind == '1' || order.chOrderKind == 'U');
		assert(order.chFunctionCode == 'B' || order.chFunctionCode == 'S');
		...
		if (pOrder != pOrderEnd)
		{
			if (pTrans != pTransEnd)
			{
				...
				else
				{
					...
					// 连续成交
					while (pTrans != pTransEnd && pTrans->nTime <= nNextTime 
						&& pTrans->nIndex < pOrder->nOrder && pTrans->chFunctionCode != 'C')
						SZE_Trans(*pTrans++);
					return true;
				}
			}
	...
	void SetFence10()
	{
		auto high = std::max((iLastPrice * 110 + 5000) / 10000 * 100, iLastPrice + 100);
		auto low = std::min((iLastPrice * 90 + 5000) / 10000 * 100, iLastPrice - 100);
	...
	void SetFence2()
	{
		auto buyRef = coalesce(sellBook.BestPrice(), buyBook.BestPrice(), iLastPrice);
		buyBook.SetFrontFence(std::max((buyRef * 102 + 5000) / 10000 * 100, buyRef + 100));
		buyBook.SetBackFence(0);

		auto sellRef = coalesce(buyBook.BestPrice(), sellBook.BestPrice(), iLastPrice);
		sellBook.SetFrontFence(std::min((sellRef * 98 + 5000) / 10000 * 100, sellRef - 100));
		sellBook.SetBackFence(INT64_MAX);
	}
...
public:
	...
	{
		return (iTurnover + 5000) / 10000;
	}

	int64_t TotalBidVol() const
	{
		return buyBook.TotalVolume();
	}

	int64_t TotalAskVol() const
	{
	...
	{
		return buyBook.TotalVolume() ? buyBook.TotalValue() / buyBook.TotalVolume() : 0;
	}

	int64_t WeightedAvgAskPrice() const
	{
		return sellBook.TotalVolume() ? sellBook.TotalValue() / sellBook.TotalVolume() : 0;
	}

	uint32_t BidDepth() const
	{
		return buyBook.Depth();
	}

	auto BidBegin() const
	{
		return buyBook.Begin();
	}
	...
	uint32_t BidHeadDepth() const
	{
		return buyBook.HeadDepth();
	}

	auto BidHeadBegin() const
	{
		return buyBook.HeadBegin();
	}

	auto BidHeadEnd() const
	{
		return buyBook.HeadEnd();
	}

	uint32_t BidTailDepth() const
	{
		return buyBook.TailDepth();
	}

	auto BidTailBegin() const
	{
		return buyBook.TailBegin();
	}

	auto BidTailEnd() const
	{
		return buyBook.TailEnd();
	}

	uint32_t AskDepth() const
	{
		return sellBook.Depth();
	}

	auto AskBegin() const
	{
		return sellBook.Begin();
	}
	...
	uint32_t AskHeadDepth() const
	{
		return sellBook.HeadDepth();
	}

	auto AskHeadBegin() const
	{
		return sellBook.HeadBegin();
	}

	auto AskHeadEnd() const
	{
		return sellBook.HeadEnd();
	}

	uint32_t AskTailDepth() const
	{
		return sellBook.TailDepth();
	}

	auto AskTailBegin() const
	{
		return sellBook.TailBegin();
	}

	auto AskTailEnd() const
	{
		return sellBook.TailEnd();
	}
	...
	bool Load(XYData& reader, const std::string & code, int date)
	{
			...
			&& transBuffer.GetSize() > 0)
		{
			...
			if (date < NEW_RULES_DAY && !SSE)
			{
				...
				{
#if 0
					for (size_t i = 0; i < marketBuffer.GetSize(); i++)
					{
						if (marketBuffer.GetData()[i].nStatus)
						{
							iLastPrice = marketBuffer.GetData()[i].nPreClose;
							NoLimitsRule = (marketBuffer.GetData()[i].nLowLimited <= 100);
							break;
						}
					}
#else
					auto i = marketBuffer.GetSize() - 1;
					iLastPrice = marketBuffer.GetData()[i].nPreClose;
					NoLimitsRule = (marketBuffer.GetData()[i].nLowLimited <= 100);
#endif
				}
	...
	bool Step()
	{
		assert(nTime < MKT_START || nTime >= MKT_FINAL
			|| BidBegin() == BidEnd() || AskBegin() == AskEnd()
			|| BidBegin()->first < AskBegin()->first);
	...
	bool Next(int period)
	{
		...
		while (nNextTime <= roundup(nTime, period))
			Step();
	...
	void Verify(const TDF_MARKET_DATA& market) const
	{
		if (Volume() != market.iVolume)
			std::cout << "Volume mismatch: " << Volume() << " " << market.iVolume << std::endl;

		if (Turnover() != market.iTurnover)
			std::cout << "Turnover mismatch: " << Turnover() << " " << market.iTurnover << std::endl;

		if (TotalBidVol() != market.nTotalBidVol)
			std::cout << "TotalBidVol mismatch: " << TotalBidVol() << " " << market.nTotalBidVol << std::endl;

		if (TotalAskVol() != market.nTotalAskVol)
			std::cout << "TotalAskVol mismatch: " << TotalAskVol() << " " << market.nTotalAskVol << std::endl;

		if (WeightedAvgBidPrice() != market.nWeightedAvgBidPrice)
			std::cout << "WeightedAvgBidPrice mismatch: " << WeightedAvgBidPrice() << " " << market.nWeightedAvgBidPrice << std::endl;

		if (WeightedAvgAskPrice() != market.nWeightedAvgAskPrice)
			std::cout << "WeightedAvgAskPrice mismatch: " << WeightedAvgAskPrice() << " " << market.nWeightedAvgAskPrice << std::endl;
		...
		for (int i = 0; i < 10; i++)
		{
			if (buy != buyBook.End())
			{
				if (buy->first != market.nBidPrice[i] || buy->second != market.nBidVol[i])
				{
					std::cout << "Buy mismatch: " << i << " (" << buy->first << "," << buy->second << ") ("
						<< market.nBidPrice[i] << "," << market.nBidVol[i] << ")" << std::endl;
				}
				buy++;
			}
			else
			{
				if (market.nBidVol[i] != 0)
				{
					std::cout << "Buy mismatch: " << i << " (0,0) ("
						<< market.nBidPrice[i] << "," << market.nBidVol[i] << ")" << std::endl;
				}
			}
		}
		...
		for (int i = 0; i < 10; i++)
		{
			...
			{
				if (sell->first != market.nAskPrice[i] || sell->second != market.nAskVol[i])
				{
					std::cout << "Sell mismatch: " << i << " (" << sell->first << "," << sell->second << ") ("
						<< market.nAskPrice[i] << "," << market.nAskVol[i] << ")" << std::endl;
				}
				sell++;
			}
			else
			{
				if (market.nAskVol[i] != 0)
				{
					std::cout << "Sell mismatch: " << i << " (0,0) ("
						<< market.nAskPrice[i] << "," << market.nAskVol[i] << ")" << std::endl;
				}
			}
		}
	...
	static size_t CompareBuys(const int64_t* begNew, size_t sizeNew,
		const int64_t* begOld, size_t sizeOld, int64_t* result, bool showUnchanged)
	{
		return OBCompare<std::greater<int64_t>>(begNew, sizeNew, begOld, sizeOld, result, showUnchanged);
	}

	static size_t CompareSells(const int64_t* begNew, size_t sizeNew,
		const int64_t* begOld, size_t sizeOld, int64_t* result, bool showUnchanged)
	{
		return OBCompare<std::less<int64_t>>(begNew, sizeNew, begOld, sizeOld, result, showUnchanged);
	}
};
